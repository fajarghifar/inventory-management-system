...
final class CustomerTable extends PowerGridComponent
{
    ...
    public string $tableName = 'customer-table-yzl1qp-table';
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('name')
            ->add('name_formatted', fn(Customer $model) => ucwords(strtolower($model->name)))
            ->add('email')
            ->add('phone')
            ->add('address')
            ->add('created_at')
            ->add('created_at_formatted', fn(Customer $model) => Carbon::parse($model->created_at)->format('d/m/Y H:i'));
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Email', 'email')
                ->sortable()
                ->searchable()
                ->bodyAttribute('text-gray-600 hover:underline'),
    ...
    public function actions(Customer $row): array
    {
        return [
            ...
            Button::add('edit')
                ...
                ->route('customers.edit', ['customer' => $row->id])
            ...
            Button::add('delete')
                ...
                ->attributes(['wire:confirm' => 'Are you sure you want to delete this customer?'])
                ->tooltip('Delete Customer'),
        ];
    }

    #[\Livewire\Attributes\On('delete')]
    public function delete($rowId, \App\Services\CustomerService $customerService): void
    {
        ...
        if ($customer) {
            try {
                ...
                $this->dispatch('toast', message: 'Failed to delete customer: ' . $e->getMessage(), type: 'error');
            }
        }
    }
}
